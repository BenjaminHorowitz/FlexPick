/*
  # Add rankings column to user_picks table

  1. Changes
    - Add `rankings` (jsonb) column to user_picks table
  ...
  2. Important Notes
    - This allows users to assign confidence levels (1, 2, 3) to their picks
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name = 'user_picks' AND column_name = 'rankings'
  ) THEN
    ALTER TABLE user_picks ADD COLUMN rankings jsonb DEFAULT '{}'::jsonb;
  END IF;
END $$;
